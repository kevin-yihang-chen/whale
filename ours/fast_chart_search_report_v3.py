"""Reconstruct V3 E1 and all E2/E3 decisions from a complete real candidate archive.
...
from .chart_selection_protocol import acceptance_for, selection_modes
...
def counts(item):
    audit = item['audit']
    pair_count = len(audit.correctness)
    h_count = len(item['h']['records'])
    if pair_count < 1 or h_count != item['h']['examples']:
        raise ValueError('Require all plan-bound H and C-pair outcomes')
    correct = audit.correctness
    zero, one, both = [sum(a+b == k for a, b in correct) for k in range(3)]
    single = sum(a+b for a, b in correct)
    if both != single-pair_count+zero:
    ...
    return {'candidate': item['candidate'].name, 'H_correct': sum(r['correct'] for r in item['h']['records']),
        'H_total': h_count, 'mean_turns': item['candidate'].mean_turns,
        'C_single_correct': single, 'C_images': 2*pair_count, 'C_both_correct': both, 'C_pairs': pair_count,
...
def latex(report):
    ...
    for row in report['rows']:
        lines.append(f"{row['candidate']} & {row['H_correct']}/{row['H_total']} & "
                     f"{row['C_single_correct']}/{row['C_images']} & "
                     f"{row['C_both_correct']}/{row['C_pairs']}"+r'\\')
    ...
    labels = {'whale': 'WHALE', 'veto': 'VETO', 'marginal_gate': 'Marginal gate',
              ...
              'point_gate':'Point-estimate paired gate'}
