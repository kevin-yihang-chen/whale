    ...
    class VisualTrajectoryRecordingTrainer(module.DisaggregatedRayTrainer):
        ...
        def _record_visual_batch(self, batch, kind):
            ...
            for i, score in enumerate(scores):
                ...
                if self.config.data.get('visual_answer_protocol') is not None:
                    from .chart_answer_protocol import PROTOCOL, verify
                    if self.config.data.visual_answer_protocol != PROTOCOL:
                        raise ValueError('Unknown shared training answer protocol')
                    correct = verify(answer, truth)
                else:
                    correct = binary_answer_verifier(answer, truth)
