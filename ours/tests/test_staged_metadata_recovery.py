...
from ours.staged_metadata_recovery import (FAILURE, MetadataRecoveredSearch, check_amendment,
...
class MetadataTests(unittest.TestCase):
    def test_lossless_alias_mapping_and_ambiguity_rejection(self):
        payload=[{'harness':'h4','path':'harnesses/h4/harness.py','hypothesis':'unverified prose'}]
        ...
        for entry in ({'harness':'h4','slot':'h5'},{'id':'h6'},{'name':'h4','parent':'h99'},
    ...
    def test_format_compatibility_does_not_allow_protected_or_extra_files(self):
        for case in ('valid','protected','extra','symlink'):
            with self.subTest(case=case),tempfile.TemporaryDirectory() as tmp:
                ...
                write_json(view/'pending_eval.json',[{'harness':'h4'}])
                ...
                if case=='valid':
                    ...
                    self.assertEqual(json.loads((view/'pending_eval.json').read_text()),[{'harness':'h4'}])
                else:
...
class NativeRecoveryTests(unittest.TestCase):
    def test_two_interruptions_preserve_original_rounds_and_perfect_stop(self):
        ...
        for perfect in (False,True):
            ...
            for recover in (False,True):
                with self.subTest(perfect=perfect,recover=recover),tempfile.TemporaryDirectory() as tmp:
                    ...
                    def proposer(**kw):
                        ...
                        payload=({'candidates':[{'slot':n} for n in names]} if kw['iteration']==1 else [{'harness':n} for n in names])
                    ...
                    def command(argv,**kw):
                        ...
                        solved=32 if perfect and name=='h5' else int(name[1:])
                    ...
                    with patch('ours.staged_search.subprocess.check_output',command), \
                        ...
                        if not recover:
                            ...
                            with patch('ours.staged_metadata_recovery.check_plan',return_value=plan), \
                                ...
                                prepare(plan_path,path2)
                                candidate=trial/'proposal-2/proposer-workspace/harnesses/h4/harness.py'
                                saved=candidate.read_bytes();candidate.write_bytes(saved+b'\n# tampered\n')
                                with self.assertRaisesRegex(ValueError,'Changed interrupted trial evidence'):check_amendment(path2)
                                candidate.write_bytes(saved)
                                amendment,checked,original=check_amendment(path2)
                            MetadataRecoveredSearch(checked,original,submit=True).run_recovered(amendment,path2)
                            self.assertEqual(evidence,{p:tree_hashes(Path(p)) for p in evidence})
                            self.assertEqual((trial/'search/logs/iteration_001/comparison.json').read_bytes(),completed)
                            self.assertEqual(json.loads((trial/'recovery-2/proposal-replay.json').read_text())['new_api_calls'],0)
                            with self.assertRaises(FileExistsError):MetadataRecoveredSearch(checked,original).run_recovered(amendment,path2)
            ...
            self.assertEqual(len(outcomes[1][1]),2 if perfect else 5)
            self.assertEqual(len(outcomes[1][2]),7 if perfect else 16)
            print(json.dumps({'kind':'native_second_metadata_recovery_fixture','status':'PASS',
