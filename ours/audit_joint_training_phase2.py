"""Audit every second-phase joint-training trajectory and native request.
...
from .joint_resume_receipts import verify_restore_receipts
from .native_phase_resume import check_configuration
...
async def audit(plan_path,directory):
    ...
    require(sorted(p.name for p in directory.glob('batch-*.jsonl.gz'))==['batch-2.jsonl.gz'],'Missing or extra sampled batch')
    header,rows,receipt=load_batch(directory,step=2);job=header['context']['job_id']
    ...
    start_path=Path(f'results/controlled-joint-phase2-{job}/start.json')
    start=json.loads(start_path.read_text())
    require(start['status']=='PASS' and start['plan_sha256']==file_sha256(plan_path) and
            start['condition']==plan['condition'] and start['seed']==plan['seed'] and start['phase']==2 and
            start['run_name']==run_name(plan,job),'Wrong joint start receipt')
    restoration=verify_restore_receipts(plan,plan_path,job,start_path.parent)
    ...
    require(env['CHESS_PUZZLE_HARNESS_PATH']==plan['harness'],'Wrong phase-two harness')
    ...
    expected=plan['native_dataset_order']['planned_batch_ids'][1]
    ...
    artifacts=[*sorted(directory.glob('requests-*.jsonl')),directory/receipt['file'],directory/'batch-2.receipt.json',start_path]
    artifacts.extend(Path(p) for p in restoration['artifact_sha256'])
    sources=('ours/joint_resume_receipts.py','ours/native_resume_observation.py','ours/native_phase_resume.py','ours/audit_joint_training_phase2.py','ours/joint_training_phase2.py','ours/audit_alternation_training.py',
             'ours/audit_native_training_batch.py','ours/native_training_trace.py')
    return {'kind':'controlled_joint_phase2_batch_audit','status':'PASS','job_id':job,'condition':plan['condition'],
        'seed':plan['seed'],'phase':2,'plan_sha256':file_sha256(plan_path),'context':context,'trajectories':64,
        ...
        'trajectory_checks':checked,'request_accounting':pool.accounting,'restoration':restoration,
        'harness_sha256':plan['harness_sha256'],'resume_checkpoint':plan['resume_checkpoint'],
        ...
        'new_model_calls':0,'limitations':['Complete recorded reply/token/mask replay and independent board checks.',
            'Restoration receipts and second-batch rows are checked; optimizer completion and final checkpoint require terminal audit.',
...
if __name__=='__main__':
    ...
    args=parser.parse_args();require(not args.output.exists(),'Preserve existing second-phase audit')
