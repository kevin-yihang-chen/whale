"""Independent complete MH audit before a staged score enters native search."""
...
def audit_evaluation(plan, plan_path, private):
    ...
    examples = {e.example_id: e for e in runner._read_examples(plan['dataset'], limit=32, seed=plan['seed'])}
    all_outputs, records = {}, []
    ...
    require(set(all_outputs) == set(examples) and len(records) == 32, 'Incomplete global coverage')
    ...
    return {'kind':'staged_mh_evaluation_audit','status':'PASS','plan_sha256':file_sha256(plan_path),
        'harness_sha256':request['harness_sha256'],'seed':plan['seed'],'examples':32,
        ...
        'limitations':['Complete MH optimization-set audit, not heldout improvement.',
