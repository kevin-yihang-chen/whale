...
def replay(plan_path, partition, incoming_path, output):
    ...
    if role == 'V':
        assert partition == 'V' and plan['kind'] == 'native_visual_post_training_case'
        assert plan['role'] == 'V' and plan['mode'] == 'direct'
        data = {'manifest': plan['manifest'], 'manifest_sha256': plan['manifest_sha256']}
        result_directory = 'evaluation'
    else:
        assert plan['kind'] == 'visual_search_candidate_evaluation'
        data = plan['partitions'][role]
        result_directory = partition
    assert file_sha256(Path(data['manifest'])) == data['manifest_sha256']
    if role == 'H':
        manifest, rows = single_rows(data['manifest'])
        pairs = None
    else:
        manifest, pairs, rows = pair_inputs(data['manifest'])
    assert manifest['role'] == role
    truth = {r['visual_sample_id']: r['reward_model']['ground_truth'] for r in rows}
    result_path = Path(plan['output']) / result_directory / 'result.json'
    ...
    if pairs is not None:
        by_sample = {r['sample_id']: r for r in outcomes}
        report['paired_counts'] = {'pairs': len(pairs)}
        for label in ('candidate', 'incoming_parser'):
            report['paired_counts'][label + '_both_correct'] = sum(all(
                by_sample[fingerprint({'pair_id': pair.pair_id, 'side': side})][label + '_correct']
                for side in (0, 1)) for pair in pairs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--partition', choices=('H', 'C', 'H-repeat', 'C-repeat', 'V'), required=True)
