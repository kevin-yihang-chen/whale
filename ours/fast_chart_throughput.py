...
@contextmanager
def input_preparation_timing():
    """Observe E1 input preparation without changing its arguments or output."""
    from . import native_visual_service, visual_native_evaluation
    targets = ((visual_native_evaluation, 'write_pair_parquet'),
               (native_visual_service, 'dataset_for'))
    measurements = {}
    originals = []
    def instrument(original, name):
        ...
        def measured(*args, **kwargs):
            ...
            measurements.setdefault(name, []).append(time.monotonic()-started)
            return value
        return measured
    try:
        for module, name in targets:
            ...
            setattr(module, name, instrument(original, name))
        yield measurements
    finally:
...
def preparation_seconds(record, plan_sha256, result):
    """Require two complete measured stages bound to this real evaluation."""
    if (record['status'] != 'COMPLETE_INPUT_PREPARATION_TIMING' or
            record['plan_sha256'] != plan_sha256 or
            str(record['job_id']) != str(result['job_id']) or record['images'] != 512):
        raise ValueError('Input preparation timing differs from its evaluation')
    stages = record['stage_seconds']
    if set(stages) != {'write_pair_parquet', 'dataset_for'}:
        raise ValueError('Input preparation stages are incomplete')
    if any(len(values) != 1 or not math.isfinite(values[0]) or values[0] < 0
           for values in stages.values()):
        raise ValueError('Require one finite duration per input preparation stage')
    seconds = sum(values[0] for values in stages.values())
    if seconds > result['timing_seconds']['loading'] + 1e-6:
        raise ValueError('Input preparation exceeds the enclosing loading duration')
    return seconds


async def run_case(path):
    plan = check_case(path)
    with input_preparation_timing() as stages:
        await evaluate(plan, path)
    result = read(Path(plan['output'])/'result.json')
    record = {'status': 'COMPLETE_INPUT_PREPARATION_TIMING',
        'plan_sha256': file_sha256(path), 'job_id': os.environ['SLURM_JOB_ID'],
        'images': plan['bounds']['images'], 'stage_seconds': stages,
        'scope': 'Materialization and dataset preparation already included in loading; not additional GPU time.'}
    preparation_seconds(record, file_sha256(path), result)
    write_new(Path(plan['output'])/'input-preparation-timing.json', record)
...
def summarize(path):
    ...
    for n, (plan_path, result_path) in paths.items():
        ...
        if n != '8':
            timing_path = result_path.parent/'input-preparation-timing.json'
            measurements[n]['input_preparation_seconds_for_512'] = preparation_seconds(
                read(timing_path), file_sha256(plan_path), result)
            measurements[n]['input_preparation_timing'] = str(timing_path)
            evidence[str(timing_path)] = file_sha256(timing_path)
...
if __name__ == '__main__':
    ...
    if args.phase == 'prepare':
        ...
        asyncio.run(run_case(args.plan.resolve()))
    else:
