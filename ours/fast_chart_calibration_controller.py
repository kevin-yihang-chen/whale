...
def completed_artifact(directory,path,*,timeout_seconds=300):
    """Allow bounded shared-filesystem visibility lag after scheduler completion."""
    path=Path(path)
    try:
        return read(path)
    except (FileNotFoundError,json.JSONDecodeError) as exc:
        import hashlib
        name=hashlib.sha256(str(path).encode()).hexdigest()[:20]
        marker=Path(directory)/f'artifact-visibility-wait-{name}.json'
        if not marker.exists():
            write_new(marker,{'artifact':str(path),'first_seen_unix':time.time(),
                'error_type':type(exc).__name__,'timeout_seconds':timeout_seconds})
        evidence=read(marker)
        if evidence['artifact']!=str(path):raise ValueError('Visibility marker identity differs')
        if time.time()-evidence['first_seen_unix']>=timeout_seconds:
            raise RuntimeError(f'Completed allocation artifact unavailable after bounded wait: {path}') from exc
        return None
...
def advance(directory,h0_plan,recovery=None):
    ...
    h0_result=completed_artifact(directory,calibration)
    if h0_result is None:return {'status':'WAITING_COMPLETED_H0_ARTIFACT'}
    if h0_result['status']!='COMPLETE_SHARED_H0_CALIBRATION':raise ValueError('Incomplete h0 calibration')
    results={}
    for rate in RATES:
        ...
        training_result=Path(read(train_plan)['output'])/'execution-result.json'
        if completed_artifact(directory,training_result) is None:
            return {'status':'WAITING_COMPLETED_TRAINING_ARTIFACT','rate':rate}
        ...
        result=completed_artifact(directory,record_path)
        if result is None:return {'status':'WAITING_COMPLETED_FOLLOWUP_ARTIFACT','rate':rate}
...
def start_receipt(directory,h0_plan,recovery,resume):
    record={'pid':os.getpid(),'h0_plan':str(h0_plan),
    ...
    target=directory/'controller-start.json'
    if resume:
        old=read(target);failure=directory/'controller-failure.json'
        if (directory/'result.json').exists() or not failure.exists():
            raise ValueError('Resume requires an incomplete, previously diagnosed controller')
        for key in ('h0_plan','h0_plan_sha256','recovery','recovery_sha256','scope'):
            if old[key]!=record[key]:raise ValueError(f'Resume changed controller scope: {key}')
        record.update(original_start_sha256=file_sha256(target),
            preserved_failure_sha256=file_sha256(failure),resume=True)
        target=directory/f'controller-resume-{uuid.uuid4().hex}.json'
    write_new(target,record)
    return target


def run(directory,h0_plan,recovery=None,*,resume=False):
    ...
    with (directory/'controller.lock').open('a') as lock:
        ...
        start_receipt(directory,h0_plan,recovery,resume)
        try:
            ...
            failure_path=directory/'controller-failure.json'
            if failure_path.exists():failure_path=directory/f'controller-failure-{uuid.uuid4().hex}.json'
...
if __name__=='__main__':
    ...
    a=p.parse_args();run(a.output,a.h0_plan,a.recovery,resume=a.resume)
