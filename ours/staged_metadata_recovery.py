"""Resume E3 at the interrupted second proposal with lossless metadata mapping.

Candidate bytes and all previously audited evaluations are reused. The original
selection loop continues at iteration two; no paid proposal is regenerated.
...
FAILURE = 'Expected allocated candidate metadata'
SOURCE = Path(__file__)
ORIGINAL_NORMALIZE = scoped_proposer.normalize_candidates
PREVIOUS_SOURCE = Path('ours/staged_search_recovery.py')


def normalize_metadata(payload, names, parents):
    """Wrap a bare list and resolve agreeing name/slot/harness/id aliases only."""
    ...
    for entry in payload['candidates']:
        ...
        aliases = [entry[key] for key in ('name','slot','harness','id') if key in entry]
...
def interruption_snapshot(plan, plan_path):
    ...
    require(state['status']=='FAILED' and state['error_type']=='ValueError' and state['error']==FAILURE,
            'Recovery requires the recorded second-proposal metadata interruption')
    try:
    ...
    require(not (root/'result.json').exists() and not (root/'recovery-2').exists(), 'Already resumed or completed')
    previous = root/'recovery-1/amendment.json'
    prior = json.loads(previous.read_text())
    require(prior['kind']=='staged_search_report_path_recovery' and
            prior['original_plan_sha256']==file_sha256(plan_path) and
            prior['source_sha256']=={str(PREVIOUS_SOURCE.resolve()):file_sha256(PREVIOUS_SOURCE)},
            'Changed first recovery provenance')
    evaluations = json.loads((root/'evaluations.json').read_text())
    require([v['harness'] for v in evaluations]==['h0','h1','h2','h3'] and
            sorted(p.name for p in (root/'evaluations').iterdir())==['h0','h1','h2','h3'],
            'Expected exactly four audited evaluations')
    controller = StagedSearch(plan, plan_path)
    controller.evaluations = evaluations
    controller.verify_public()
    comparison = json.loads((root/'search/logs/iteration_001/comparison.json').read_text())
    require(comparison['iteration']==1 and comparison['early_stop'] is None and
            (root/'search/logs/accepted_harness.txt').read_text().strip()==comparison['accepted_harness'],
            'First-round selection is incomplete or should have stopped')
    require(sorted(p.name for p in (root/'search/harnesses').iterdir())==['h0','h1','h2','h3'],
            'Second-round candidate already imported')
    request = json.loads((root/'proposal-request-2.json').read_text())
    require(request['iteration']==2 and request['next_names']==['h4','h5','h6'] and
            Path(request['run_dir'])==root/'search', 'Wrong second proposal allocation')
    receipt = json.loads((root/'proposal-2/proposer-result.json').read_text())
    require(receipt['exit_code']==0 and receipt['iteration']==2 and
            receipt['allocated_slots']==request['next_names'] and receipt['budget']['unresolved_calls']==0,
            'Incomplete second paid proposal')
    search = tree_hashes(root/'search')
    before = {k:v for k,v in search.items() if k!='pending_eval.json'}
    view = root/'proposal-2/proposer-workspace'
    with tempfile.TemporaryDirectory(prefix='whale-metadata-review-') as tmp:
        ...
        allowed = check_metadata_proposal(copy,before,request['next_names'],2,Path(tmp)/'normalization')
        payload = normalize_metadata(json.loads((copy/'pending_eval.json').read_text()),request['next_names'],set(['h0','h1','h2','h3']))
    ...
    return {'state':state,'search_sha256':search,
        ...
        'receipt_sha256':file_sha256(root/'proposal-2/proposer-result.json'),
        'request_sha256':file_sha256(root/'proposal-request-2.json'),
        'evaluations_sha256':file_sha256(root/'evaluations.json'),
        'native_config_sha256':file_sha256(root/'native-config.json'),
        'previous_recovery_sha256':tree_hashes(root/'recovery-1'),
        'raw_sse_sha256':tree_hashes(root/'proposal-2/raw-sse')}
...
def prepare(plan_path, amendment_path):
    ...
    amendment = {'kind':'staged_search_second_metadata_recovery',
        ...
        'change':'Wrap a bare candidate list and map agreeing name/slot/harness/id aliases; retain all original fields.',
        'recovery':'Original run_evolve at start_iteration=2, iterations=4; replay completed proposal 2 once.',
        'new_calls_for_replayed_proposal':0,'new_model_calls_for_reused_evaluations':0,
        'original_plan_modified':False,
        'limitations':['Explicit execution amendment after proposal generation and before second-round evaluation.',
...
def check_amendment(path):
    ...
    require(amendment['kind']=='staged_search_second_metadata_recovery','Wrong recovery amendment')
...
def replay_proposal(root,recovery,snapshot,**kwargs):
    ...
    require(serial==json.loads((root/'proposal-request-2.json').read_text()),'Second proposal request changed')
    original = root/'search'
    require(tree_hashes(original)==snapshot['search_sha256'],'Native continuation changed proposal inputs')
    source = root/'proposal-2/proposer-workspace'
    ...
    allowed = check_metadata_proposal(view,before,kwargs['next_names'],2,recovery/'report-normalization')
    payload = normalize_metadata(json.loads((view/'pending_eval.json').read_text()),kwargs['next_names'],set(['h0','h1','h2','h3']))
    ...
    write_json(recovery/'proposal-replay.json',{'status':'PASS','iteration':2,'imported':allowed,
    ...
    return SimpleNamespace(show=lambda:print('Replayed the exact completed second proposal; zero new API calls.',flush=True))


class MetadataRecoveredSearch(StagedSearch):
    def run_recovered(self,amendment,amendment_path):
        recovery = self.root/'recovery-2'
        ...
        self.state('RECOVERING_SECOND_PROPOSAL',amendment_sha256=file_sha256(amendment_path),budget=journal.summary())
        args = native_arguments(self.plan,self.root)
        args.start_iteration,args.iterations = 2,4
        ...
        with native_search_context(self.plan['condition'],self.root,self.plan['incoming_harness']) as (native,benchmark,provenance):
            ...
            def propose(**kwargs):
                ...
                require(3<=number<=5,'Outside original remaining rounds')
            ...
            try:
                ...
                write_json(self.root/'result.json',{'status':'COMPLETED_NATIVE_SEARCH','plan_sha256':file_sha256(self.plan_path),
                    ...
                    'previous_recovery_amendment_sha256':file_sha256(self.root/'recovery-1/amendment.json'),
