...
def verifier_identity():
    return fingerprint({name: file_sha256(Path(__file__).with_name(name))
...
async def evaluate_pairs(manager, dataset, *, manifest_path, identity, output, batch_size):
    ...
    if identity.audit_data_sha256 != manifest['audit_data_sha256'] or identity.verifier_sha256 != verifier_identity():
    ...
    try:
        for offset in range(0, len(dataset), batch_size):
            ...
            for index, sample_id in enumerate(actual_ids):
                ...
                reward = int(binary_answer_verifier(committed, expected[sample_id]['reward_model']['ground_truth']))
