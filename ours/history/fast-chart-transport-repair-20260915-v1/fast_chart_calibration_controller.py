...
def advance(directory,h0_plan):
    ...
    for rate in RATES:
        ...
        train_plan=directory/(name+'-training-plan.json')
        ...
        if state is None:
            command(directory,name+'-submit',['ours.fast_chart_submit','--kind','training','--phase','setup','--plan',str(train_plan)])
        ...
        followup=directory/(name+'-followup-plan.json')
    ...
    with (directory/'controller.lock').open('a') as lock:
        ...
        try:
            while not (directory/'result.json').exists():
                report=advance(directory,h0_plan)
            ...
            write_new(directory/'controller-failure.json',{'status':'STOPPED_REQUIRES_DIAGNOSIS',
...
if __name__=='__main__':
    ...
    p.add_argument('--h0-plan',type=Path,required=True);a=p.parse_args();run(a.output,a.h0_plan)
