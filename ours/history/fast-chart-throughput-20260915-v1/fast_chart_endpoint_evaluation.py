...
async def external(manager,dataset,plan,output,expected):
    ...
    for offset in range(0,len(dataset),8):
        items=[dataset[i] for i in range(offset,offset+8)];batch=DataProto.from_single_dict(collate_fn(items));batch.meta_info.update(validate=True)
        result=await manager.generate_sequences(batch);archive=output/f'batch-{offset//8:04d}.pkl'
        ...
        if len(set(ids))!=8 or set(ids)!={x['visual_sample_id'] for x in items}:raise ValueError('External batch coverage differs')
        records=[]
        ...
        rows.extend(records);write_new(output/f'batch-{offset//8:04d}.json',records)
    dataset.visual_harness.unchanged()
...
async def run(plan,path):
    ...
    try:
        ...
        if expected is not None:result=await external(manager,dataset,plan,out/'evaluation',expected)
            ...
            _,result=await evaluate_pairs(manager,dataset,manifest_path=plan['manifest'],identity=identity,output=out/'evaluation',batch_size=8)
